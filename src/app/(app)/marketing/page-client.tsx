...
function toPackageForm(pkg: MarketingPackageDto): PackageForm {
  return {
    nome: pkg.nome,
...
function PackageEditor({
  ...
  return (
    <>
      <input value={form.nome} onChange={(e) => setForm((p) => ({ ...p, nome: e.target.value }))} placeholder="Nome do pacote" className="w-full rounded-2xl border border-border bg-surface-muted px-4 py-3 outline-none focus:ring-2 focus:ring-accent" />
      <textarea rows={3} value={form.descricao} onChange={(e) => setForm((p) => ({ ...p, descricao: e.target.value }))} placeholder="Descrição comercial" className="w-full rounded-2xl border border-border bg-surface-muted px-4 py-3 outline-none focus:ring-2 focus:ring-accent" />
      <textarea rows={5} value={form.entregaveis} onChange={(e) => setForm((p) => ({ ...p, entregaveis: e.target.value }))} placeholder="Entregáveis, um por linha" className="w-full rounded-2xl border border-border bg-surface-muted px-4 py-3 outline-none focus:ring-2 focus:ring-accent" />
      <div className="grid gap-3 md:grid-cols-2">
        <input type="number" min="0" step="0.01" value={form.investimento} onChange={(e) => setForm((p) => ({ ...p, investimento: e.target.value }))} placeholder="Investimento" className="w-full rounded-2xl border border-border bg-surface-muted px-4 py-3 outline-none focus:ring-2 focus:ring-accent" />
        <input type="number" min="0" value={form.ordem} onChange={(e) => setForm((p) => ({ ...p, ordem: e.target.value }))} placeholder="Ordem" className="w-full rounded-2xl border border-border bg-surface-muted px-4 py-3 outline-none focus:ring-2 focus:ring-accent" />
      </div>
      <textarea rows={4} value={form.cronograma} onChange={(e) => setForm((p) => ({ ...p, cronograma: e.target.value }))} placeholder="Cronograma comercial" className="w-full rounded-2xl border border-border bg-surface-muted px-4 py-3 outline-none focus:ring-2 focus:ring-accent" />
...
export default function MarketingPageClient() {
  ...
  const [inlineForm, setInlineForm] = useState<PackageForm>(initialForm);
  ...
  function startInlineEdit(pkg: MarketingPackageDto) {
    setInlineEditId(pkg.id);
    setInlineForm(toPackageForm(pkg));
    setError("");
  }
  ...
  async function handleCreate(event: FormEvent<HTMLFormElement>) {
    ...
    setSaving(true);
    setError("");
    const deliverables = form.entregaveis.split("\n").map((item) => item.trim()).filter(Boolean);
    ...
    if (!response.ok) {
      ...
      showToast({ tone: "error", title: "Falha ao salvar pacote" });
      setSaving(false);
      return;
    }
    ...
    setSaving(false);
    closeCreateModal();
    showToast({ tone: "success", title: "Pacote criado", message: "A proposta comercial foi atualizada com sucesso." });
  }

  async function saveInlinePackage(id: string) {
    setInlineSaving(true);
    setError("");
    const deliverables = inlineForm.entregaveis.split("\n").map((item) => item.trim()).filter(Boolean);
    const response = await fetch(`/api/marketing-packages/${id}`, {
      method: "PATCH",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
        ...
        entregaveis: deliverables,
        investimento: Number(inlineForm.investimento),
    ...
    if (!response.ok) {
      const data = await response.json().catch(() => ({}));
      setError(data.error ?? "Não foi possível atualizar o pacote.");
      showToast({ tone: "error", title: "Falha ao atualizar pacote" });
  ...
  async function handleDelete(id: string) {
    ...
    if (!response.ok) {
      ...
      showToast({ tone: "error", title: "Falha ao remover pacote" });
      return;
    }
    ...
    <>
      <section className="space-y-6">
        <header className="rounded-[32px] border border-white/70 bg-[linear-gradient(135deg,#ffffff_0%,#fff9f5_48%,#f2f7ff_100%)] p-8 shadow-[0_18px_48px_rgba(15,23,42,0.08)]">
          <p className="text-xs font-semibold uppercase tracking-[0.2em] text-slate-500">Marketing</p>
        ...
        <div className="space-y-6">
          <div className="rounded-[32px] border border-border bg-surface p-6 shadow-sm">
            <div className="grid gap-4 md:grid-cols-[1fr_auto] md:items-end">
              <div>
                <label className="mb-1 block text-xs font-semibold uppercase tracking-[0.15em] text-slate-500">Evento para proposta</label>
                <select value={selectedEventId} onChange={(event) => setSelectedEventId(event.target.value)} className="w-full rounded-2xl border border-border bg-surface-muted px-4 py-3 outline-none focus:ring-2 focus:ring-accent">
                  {events.map((event) => (
          ...
          {packages.length === 0 ? (
            <div className="rounded-3xl border border-dashed border-border bg-surface p-8 text-sm text-slate-600 shadow-sm">Nenhum pacote comercial cadastrado ainda.</div>
          ) : (
            <div className="grid gap-6 xl:grid-cols-2">
              {packages.map((pkg) => {
                const isEditing = inlineEditId === pkg.id;
                return (
                  <article key={pkg.id} className="rounded-3xl border border-border bg-surface p-6 shadow-sm">
                    <div className="flex items-start justify-between gap-3">
                      <div>
                        <h3 className="text-xl font-heading text-slate-950">Pacote {pkg.nome}</h3>
                        <p className="mt-2 text-sm text-slate-600">{pkg.descricao ?? "Sem descrição"}</p>
                      </div>
                      <span className={`rounded-full px-3 py-1 text-xs font-semibold ${pkg.ativo ? "bg-emerald-100 text-emerald-700" : "bg-slate-100 text-slate-600"}`}>{pkg.ativo ? "Ativo" : "Inativo"}</span>
                    </div>

                    {isEditing ? (
                      <div className="mt-4 rounded-3xl border border-accent/20 bg-surface-muted/40 p-4">
                        <PackageEditor form={inlineForm} setForm={setInlineForm} />
                        <div className="mt-4 flex gap-2">
                          <button type="button" onClick={() => void saveInlinePackage(pkg.id)} disabled={inlineSaving} className="rounded-xl bg-accent px-4 py-3 text-sm font-semibold text-white disabled:opacity-60">{inlineSaving ? "Salvando..." : "Salvar"}</button>
                          <button type="button" onClick={cancelInlineEdit} className="rounded-xl border border-border px-4 py-3 text-sm font-medium text-slate-700">Cancelar</button>
                        </div>
                      </div>
                    ) : (
                      <>
                        <ul className="mt-4 space-y-2 text-sm text-slate-700">
                          {pkg.entregaveis.map((item) => (
                            <li key={item} className="rounded-2xl border border-border bg-surface-muted/50 px-3 py-2">{item}</li>
                          ))}
                        </ul>
                        <div className="mt-4 grid gap-3 md:grid-cols-2">
                          <div className="rounded-2xl border border-border bg-surface-muted/50 p-4">
                            ...
                            <p className="mt-1 text-2xl font-heading text-slate-950">{Number(pkg.investimento).toLocaleString("pt-BR", { style: "currency", currency: "BRL" })}</p>
                          </div>
                          <div className="rounded-2xl border border-border bg-surface-muted/50 p-4">
                            <p className="text-sm text-slate-500">Cronograma</p>
                            <p className="mt-1 text-sm text-slate-700">{pkg.cronograma ?? "Não informado"}</p>
                          </div>
                        </div>
                        <div className="mt-4 flex gap-2">
      ...
      <BaseModal open={showCreateModal} onClose={closeCreateModal} title="Novo pacote" description="Crie um pacote sem ocupar espaço fixo na tela.">
        <form onSubmit={handleCreate} className="space-y-4">
          <div className="rounded-2xl border border-border bg-surface-muted/70 p-4">
            <p className="text-xs font-semibold uppercase tracking-[0.15em] text-slate-500">Estrutura sugerida</p>
            <div className="mt-3 flex flex-wrap gap-2">
              {["Apresentação", "Entregáveis", "Mídia", "Pós-evento"].map((tag) => (
                <span key={tag} className="rounded-full border border-border bg-white px-3 py-2 text-xs font-semibold text-slate-700">{tag}</span>
              ))}
            </div>
          </div>
...
function QuickCard({ label, value }: { label: string; value: string }) {
  return <div className="rounded-2xl border border-border bg-surface-muted/70 p-4"><p className="text-xs uppercase tracking-[0.15em] text-slate-500">{label}</p><p className="mt-2 text-lg font-heading text-slate-950">{value}</p></div>;
}
